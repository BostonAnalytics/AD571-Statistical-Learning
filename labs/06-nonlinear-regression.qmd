---
title: "Lab 06: Nonlinear Regression"
execute:
  enabled: false
  eval: false
  warning: false
...
```{r firstmod, results='asis'}
fit = lm(wage ~ poly(age, 4), data = Wage)
stargazer::stargazer(fit, type='html', summary=TRUE,report = "vc*stp",ci=TRUE)
```
...
```{r secmod, results='asis'}
fit2 = lm(wage~poly(age, 4, raw = TRUE), data = Wage)
stargazer::stargazer(fit2, type='html', summary=TRUE,report = "vc*stp",ci=TRUE)
```
...
ggplot() +
  ...
  geom_ribbon(aes(x = age_grid, 
              ...
              alpha = 0.3) +
  xlim(agelims) +
  labs(title = "Degree-4 Polynomial")+th
```
...
```{r fivemod, results='asis'}
fit_1 = lm(wage~age, data = Wage)
fit_2 = lm(wage~poly(age,2), data = Wage)
fit_3 = lm(wage~poly(age,3), data = Wage)
fit_4 = lm(wage~poly(age,4), data = Wage)
fit_5 = lm(wage~poly(age,5), data = Wage)
stargazer::stargazer(fit_1,fit_2,fit_3,fit_4,fit_5, type='html', summary=TRUE,report = "vc*stp",ci=TRUE)

```


```{r}
anova1 <- anova(fit_1,fit_2,fit_3,fit_4,fit_5)

kable(anova1,digits=4)%>% 
...
stargazer::stargazer(fit_1,fit_2,fit_3, type='html', summary=TRUE,report = "vc*stp",ci=TRUE)
```


```{r}
anova2 <- anova(fit_1,fit_2,fit_3)
...
$$log\left(\frac{Pr(Y = 1|X)}{1 âˆ’ Pr(Y = 1|X)}\right)= X\beta$$
...
_We can produce predictions and plots just as we did in the case of the polynomial fit_


```{r img3, fig.align='center'}
...
ggplot() +
  ...
  geom_ribbon(aes(x = age_grid, 
              ...
              alpha = 0.3) +
  xlim(agelims) +
  labs(title = "Step Function")+th
```


# Python companion

Polynomial terms and splines let a linear model estimate nonlinear mean functions while keeping the fitted model interpretable.
...
df = pd.read_csv("data/lab06.csv")
X = df[["x"]]
y = df["y"]
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.30, random_state=571)

poly = Pipeline([
    ("basis", PolynomialFeatures(degree=3, include_bias=False)),
    ("model", LinearRegression()),
])
poly.fit(X_train, y_train)
print("Polynomial RMSE:", mean_squared_error(y_test, poly.predict(X_test), squared=False))

spline = Pipeline([
    ("basis", SplineTransformer(n_knots=5, degree=3, include_bias=False)),
    ("model", LinearRegression()),
])
spline.fit(X_train, y_train)
print("Spline RMSE:", mean_squared_error(y_test, spline.predict(X_test), squared=False))
```
